...
/*app.all(/.+/, function(req, res, next) {
    var r = req.route;
    log.debug(req.route.method + ' ' + req.originalUrl);
    return next();
});*/
...
app.get('/', function(req, res, next){
    ...
    https.get({host: 'answers.openmrs.org', path: '/users/'+req.session.user.uid }, function(res) {
        if (res.statusCode == 200) res.locals.osqaUser = true;
...
log.trace("defined the first route");
...
app.post('/login', mid.stripNewlines, validate(), function(req, res, next){
    ...
    ldap.authenticate(username, password, function(e){
        ...
        ldap.getUser(username, function(e, userobj) {
            ...
            app.locals.clearErrors(); // keeps "undefined" from showing up in error values
            finish();
        });
...
app.get('/profile', mid.forceLogin, function(req, res, next){
    ...
    var sidebar = app.locals.sidebar;
...
// 404's
app.get('*', function(req, res, next){
    if (req.get('Accept').indexOf('text/html') > -1) { // send an HTML error page
        res.statusCode = 404;
