...
$().ready(function(){
        ...
        this.each(function(i, element){
            ...
            if ((popoverXOffset + popoverWidth) > bodyWidth) {
                var overshot = (popoverXOffset + popoverWidth) - bodyWidth,
                    currentMargin = parseInt(popover.css('right'), 10),
    ...
    var emailAddressHtml = $.parseHTML('<div class="field next">' +
            '<input type="text" name="emails" placeholder="Add another...">' +
            '<input type="radio" name="primary-email" value="">' +
            '</div>');
    var duplicate = function(){
        if ($('.multi-field.profile-emails').children('.next').length < 1) {
            var element = $(emailAddressHtml).clone(true);
            $('.multi-field.profile-emails').append(element);
            element.children('input[name="primary-email"]').
                attr('value', element.index());
            return element;
        }
        else
            return $('.multi-field.profile-emails').children('.next');
    };
    var attemptDetach = function() {
        var value = $(this).children('input[type=text]').val();
        if (!value && !$(this).hasClass('next')) {
            $(this).detach();
        }
    };
    var makePermanent = function() {
        var value = $(this).children('input[type=text]').val();
        if (value) {
    ...
    $(emailAddressHtml).focusout(attemptDetach).keyup(makePermanent);
    $('.multi-field .field').focusout(attemptDetach);

    duplicate(); // Once at page load.
