...
var system = new LDAPServer({uri: conf.ldap.server.uri, version: 3}), // system-bind operations (usual operations)
    userbinds = {}, // object of user-bind connections (auths and password changes)
    systemTimeoutNode;
...
function toLdapForm(user, op) { // Changes a user object to an LDAP array
    ...
    for (var attr in user) {
        if (attr == 'dn' || attr == 'modifyTimestamp' ||
            (attr == conf.ldap.user.password && op != 'add') ||
            (attr == conf.ldap.user.username && op != 'add') || attr == 'memberof')
                continue; // protect from a few illegal changes
...
exports.changePassword = function(user, oldPass, newPass, cb){
    if (!cb) cb = new Function();
    var username = user.username;

    if (typeof(user)=='object' && typeof(oldPass)=='string' && typeof(newPass)=='string') {

        var onceConnected = function() {
            userbinds[username].modify(user.dn, [
...
exports.getUser = function(user, cb) {
    ...
    connect(function(e){
        ...
        if ( !conf.ldap.user.usernameRegex.exec(user) ) return cb(new Error('Illegal username specified'));
            ...
            var completed = 0, userobj = {}, groups = [];
            ...
            system.search({
                ...
                attrs: conf.ldap.user.username+' '+conf.ldap.user.firstname+' '+conf.ldap.user.lastname+' '+conf.ldap.user.displayname+' '+
                    conf.ldap.user.email+' '+conf.ldap.user.secondaryemail+' objectClass modifyTimestamp'
            }, function(e,d){
                log.trace('user profile data returned');
            ...
            system.search({
                ...
                log.trace('user group data returned');
            ...
            var finish = function(){
                completed++;
                if (completed == 2) {
                    log.trace('user lookup finished');
                    cb(undefined, userobj); // FINALLY, all done
                }
            };
        }
    });
...
exports.updateUser = function(user, cb) { // updates user using modified "user" object
    var finished = 0, removedGroups = [], everyGroup = [], everyLength, groupsIterated = 0;
    if (!cb) cb = new Function();
