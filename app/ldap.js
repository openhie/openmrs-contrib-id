...
var system = new LDAPServer({
    ...
    version: 3
  }), // system-bind operations (usual operations)
...
function systemTimeout() {
  systemTimeoutNode = setTimeout(function() {
    log.debug('system connection closing');
    system.close()
  }, 60000);
}
...
exports.updateUser = function(user, cb) { // updates user using modified "user" object
  var finished = 0,
    removedGroups = new Array,
    everyGroup = new Array,
    everyLength, groupsIterated = 0;
  if (!cb) cb = new Function;
  connect(function(e) {
    if (e) return cb(e);
  ...
  var reset = function(userdn) {
    ...
    system.search({
      ...
      system.modify(userdn, [{
        ...
        vals: [conf.ldap.user.passwordResetPolicy]
      }], function(e) {
        if (e) return cb(e);
