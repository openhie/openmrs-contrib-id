...
// Connect the rendering engine. Used `create` to work in a different
// instance than the main renderer.
app.engine('hbs', hbs.__express);
app.set('view engine', 'hbs');

// A helper used in email templates. All the helper's paramaters are
// used to contrust a URL relative to the site's path.
hbs.registerHelper('resolve', function() {
    var components = _.initial(arguments),
        resolvedComponents = path.join.apply(path, components),
        resolvedUrl = url.resolve(conf.site.url, resolvedComponents);
    return new hbs.handlebars.SafeString(resolvedUrl);
});

db.define('EmailVerification', {
    verifyId: {type: db.STRING, unique: true, primaryKey: true},
    actionId: {type: db.STRING, unique: true},
    urlBase: {type: db.STRING},
    email: {type: db.STRING},
    associatedId: {type: db.STRING},
    locals: {type: db.TEXT, defaultValue: null},
    settings: {type: db.TEXT, defaultValue: null},
    timeoutDate: {type: db.DATE, defaultValue: null}
}, {instanceMethods: {
 ...
 * },callback)    receives error and EmailVerification instance
 */
exports.begin = function(settings, callback) {
    ...
    app.render(template, options, function(err, html) {
        if (err) return callback(err);
    ...
    var finishCreate = function(){
        finished++;
        if (finished == 2) callback(null, thisVerify);
    };
...
// callback return error and new verification instance.
...
exports.check = function(credential, callback){
    ...
    db.find('EmailVerification', {verifyId: credential}, function(err, instance){
        ...
        if (verify) {
            ...
            if (!verify.timeoutDate || verify.timeoutDate > Date.now()) { // still valid; has not expired
...
exports.search = function(credential, type, callback) {
    ...
    // determine whether credential is email, username, or verifyId
    if (conf.user.usernameRegex.test(credential)) terms = {associatedId: credential}; // is a user id
    else if (conf.email.validation.emailRegex.test(credential)) terms = {email: credential}; // is an email address
